...
DOWN_GE_MP_1 = 0.0079  # multiplier to determine fall of value from last peak to place buy order
...
class ScriptAnalysis:
    ...
    def set_max_peak(self):
        if len(self.summerised_peaks) > 0:
            ...
            self.highest_peak = max_peak[1]
            # print(f"======> Highest Peak: {self.highest_peak}")
            return self.highest_peak
        else:
            return None
    ...
    def do_analyse_peaks_diff_max(self):
        if len(self.summerised_peaks) == 2:
            ...
            # max_diff_of_peaks = max(diff_of_peaks)
            return round(max(diff_of_peaks), 2)
        else:
            return None
    ...
    def script_analysis(self):
        ...
        if len(self.peaks) > 0 and len(self.troughs) == 0:
            ...
            if is_down_ge(self.peaks[-1][1], self.ltp, DOWN_GE_MP_1) and self.duration_ltp_last_peak_trough() > TIME_SINCE_LTP:
                self.sell_margin = calculate_sell_margin(self.opening_price, self.ltp)
                print(
        ...
        if len(self.peaks) > 0 and len(self.troughs) > 0:
            ...
            if self.highest_peak == self.summerised_peaks[-1][1] and self.what_last == 'peak':
                print(
                    f"INFO : {self.script_name} : script_analysis() -> Script is at Day High, so wait till next trough - NOT Tradable.")
                return False
            print(
            ...
            if len(self.peaks) == 1 and len(self.troughs) == 1:
                ...
                # PT logic
                if self.what_first == 'peak' and is_up_le(self.troughs[-1][1], self.ltp, UP_LE_MP) and self.duration_ltp_last_peak_trough() > TIME_SINCE_LTP:
                    ...
                    return True
                # TP logic
                elif self.what_first == 'trough' and is_down_ge(self.peaks[-1][1], self.ltp, DOWN_GE_MP_1):
                ...
                if is_up_le(self.troughs[-1][1], self.ltp, UP_LE_MP):
                    self.sell_margin = calculate_sell_margin(self.peaks[-1][1], self.troughs[-1][1])
                    print(
                ...
                if summerised_peak_trend == 'zz':
                    ...
                    if peak_diff_max is not None and max_peak is not None and peak_diff_max < round(0.0032 * max_peak,
                        ...
                        print(
                            f"INFO : {self.script_name} : script_analysis() -> Condition-04 (any+zz logic) -> max diff amongst peaks is out of 0.32% range - NOT Tradable")
                        return False
                else:
                    print(
